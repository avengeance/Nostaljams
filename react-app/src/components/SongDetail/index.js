...
const SongDetail = () => {
  ...
  useEffect(() => {
    ...
    dispatch(LikesActions.getLikesBySongThunk(songId)).then((likes) => {
              ...
              {comments.map((comment, index) => (
                <div key={index} className="song-comments-contents">
                  <div className="comment-user-img">
                    <img
                      src={allUsers?.[comment.userId]?.userImg?.[0]?.imgUrl}
                      alt={allUsers?.[comment.userId]?.username || "Anonymous"}
                    />
                  </div>
                  <div className="comments-user-box">
                    <p className="user-comment-name">
                      <strong>
                        {allUsers?.[comment.userId]?.username || "Anonymous"}
                      </strong>
                    </p>
                    <p className="user-comment-p">{comment.comment}</p>
                  </div>
